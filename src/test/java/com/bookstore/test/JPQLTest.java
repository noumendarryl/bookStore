...
public class JPQLTest {
    ...
    public void checkRequestWithStringParameter(){
        String jpql = "SELECT b FROM Book b WHERE b.title LIKE :title"; //remplacez par votre requête
        String param="%PHP5%"; //remplacez par la valeur de type %votreChaine% (votre motif de recherche encadré par %)
        String paramName = "title";  //remplacez par le nom du paramètre tel qu'il est défini dans votre requête JPQL
    ...
    public void checkRequestWithCategoryParameter() {
        ...
        String jpql = "SELECT c FROM Category c WHERE c.parentCategory = :parentId"; //remplacez par votre requête
        try{       
